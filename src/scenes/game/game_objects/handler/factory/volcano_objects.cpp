...
namespace game::game_objects {
	std::vector<GameObject*> create_volcano_objects(uint8_t save_id, StoryState story_state) {
		...
		return {
			...
			new Enemy(map::VOLCANO, Point(60, 32), 9, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(26, 37), 9, enemies[blit::random() % enemies.size()], UP, save_id, false),
			new Enemy(map::VOLCANO, Point(3, 47), 10, enemies[blit::random() % enemies.size()], UP, save_id, false),
			new Enemy(map::VOLCANO, Point(11, 28), 10, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(39, 24), 10, enemies[blit::random() % enemies.size()], LEFT, save_id, false),
			new Enemy(map::VOLCANO, Point(58, 17), 11, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(32, 11), 11, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
