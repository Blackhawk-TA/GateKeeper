...
namespace game::game_objects {
	std::vector<GameObject*> create_volcano_objects(uint8_t save_id, StoryState story_state) {
		...
		return {
			...
			new Enemy(map::VOLCANO, Point(21, 50), 6, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(48, 45), 6, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(60, 32), 7, enemies[blit::random() % enemies.size()], DOWN, save_id, false),
			new Enemy(map::VOLCANO, Point(26, 37), 7, enemies[blit::random() % enemies.size()], UP, save_id, false),
