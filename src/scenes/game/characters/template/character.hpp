...
public:
	...
	/**
	 * A struct consisting of the data which is stored for each character
	 */
	struct Data {
		bool player_usable{};
	...
	/**
	 * Character types. Required since PicoSystem does not support Run Time Type Information
	 */
	enum CharacterType {
	};

	explicit Character(map::MapSections map_section, Point position, bool player_usable, bool inventory_usable);
	virtual ~Character() = default;

	/**
	 * Gets the signature of a character to clearly identify it.
	 * @return The signature struct of the game object consisting of the map_section and position
	 */
	Signature get_signature();

	/**
	 * Gets the savable struct of a character to restore the character data
	 * @return The savable struct of an object
	 */
	Save get_save();

	/**
	 * Loads the saved state of the character
	 * @param value The value that is loaded in the character
	 */
	virtual void load_save(Data data);

	/**
	 * Checks if the character collides with the player's next position
	...
	/**
	 * Checks if a textbox created by this character is open
	 * @return True, if a textbox is open, else false
	 */
	bool is_textbox_open();

	/**
	 * Closes the textbox of the character
	 */
	void close_textbox();

	/**
	 * Checks if the character is currently rendered
	 * @return True, if the character is currently being rendered, else false
	 */
	bool is_rendered();
	...
	/**
	 * Called when the player interacts with a character using the inventory
	 * @return True, if interaction successful, else false
	 */
	virtual bool inventory_interact();

	/**
	 * Updates the state of the character when a player moves closer to it
	 * @param next_position The next position of the player
	 */
...
protected:
	...
	/**
	 * Sets the state and tile_id of the character
	...
	/**
	 * Set the additional integer value in which data can be saved for the character
