...
namespace combat {
	void stats::draw(uint8_t health, uint8_t stamina, uint8_t level, uint8_t enemy_health, uint8_t enemy_level) {
		//Background box left
		screen.pen = Pen(0, 0, 0, 150);
		screen.rectangle(Rect(0, BOX_Y_POS, BOX_WIDTH, 32));
		screen.pen = Pen(255, 255, 255, 255);
		screen.text("Enemy Health (" + std::to_string(enemy_level) + ")", minimal_font, Rect(5, BOX_Y_POS + 4, 10, 5), false, TextAlign::left);

		//Enemy health
		screen.pen = Pen(255, 255, 255, 200);
		screen.rectangle(Rect(5, BOX_Y_POS + 20, BAR_LENGTH, 2));
		screen.pen = Pen(255, 0, 0, 255);
		screen.rectangle(Rect(5, BOX_Y_POS + 20, int(float(enemy_health) / float(Character::MAX_HEALTH) * BAR_LENGTH), 2));
		...
		screen.rectangle(Rect(screen.bounds.w - BOX_WIDTH + 5, BOX_Y_POS + 15, int(float(health) / float(Character::MAX_HEALTH) * BAR_LENGTH), 2));
