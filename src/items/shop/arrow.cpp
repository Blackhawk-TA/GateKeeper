...
Listbox::Item items::create_shop_arrow(uint8_t type_id) {
	uint8_t price = 150;
	return Listbox::Item{
		type_id,
		"ARROW",
		"Buy an arrow for " + std::to_string(price) + " gold. It can be used during fights to defend yourself against enemies.",
		"You bought an arrow for " + std::to_string(price) + " gold",
		"You can't buy this now.",
		false,
		0,
		[price] {
			Listbox::Item item = items::create_gear_item(GEAR_ARROW);
